...
##### GeneticSingleIs_GA_GAPENAL #####
generate_slurm_files_penal <- function(folder, NbFeaturesPenalty, TournamentFeaturesPenalty, delayed){
  slurm_scenari <- glue::glue("GeneticSingleIs_GA_GAPENAL_NbFeaturesPenalty{NbFeaturesPenalty*1000}_TournamentFeaturesPenalty{TournamentFeaturesPenalty}")
  res <- glue::glue(
    '#!/bin/bash
...
generate_slurm_files_test_penal <- function(folder, NbFeaturesPenalty, TournamentFeaturesPenalty, delayed){
  slurm_scenari <- glue::glue("GeneticSingleIs_GA_GAPENAL_NbFeaturesPenalty{NbFeaturesPenalty*1000}_TournamentFeaturesPenalty{TournamentFeaturesPenalty}")
...
df_exp2 <- data.frame(NbFeaturesPenalty = c(0, 1e-3, 5*1e-3, 1e-2),
                      TournamentFeaturesPenalty = c(1, rep(0, 3))) |> 
  tibble::rowid_to_column(var = "delayed")

df_exp2 |> 
  apply(MARGIN = 1,
        FUN = function(row){
          generate_slurm_files_penal(folder = "slurm_experiments/explore_ga_exp2",
                                     NbFeaturesPenalty = as.numeric(row["NbFeaturesPenalty"]),
                                     TournamentFeaturesPenalty = as.numeric(row["TournamentFeaturesPenalty"]),
                                     delayed = (as.numeric(row["delayed"])-1)*3)
        })
...
df_exp2 |> 
  apply(MARGIN = 1,
        FUN = function(row){
          generate_slurm_files_test_penal(folder = "slurm_experiments/explore_ga_exp2_test",
                                          NbFeaturesPenalty = as.numeric(row["NbFeaturesPenalty"]),
                                          TournamentFeaturesPenalty = as.numeric(row["TournamentFeaturesPenalty"]),
                                          delayed = (as.numeric(row["delayed"])-1)*3)
        })
