...
blue_palette <- viridis::mako(n = 4, end = 0.8)
red_palette <- c("black", "#d7301f", "#fc8d59", "#fdcc8a")
# puor_palette <- c("#5e3c99", "#b2abd2", "#fdb863", "#e66101")
puor_palette <- viridis::magma(n = 4, end = 0.9)
bicolor_palette <- viridis::cividis(n = 2, end = 0.85, direction = -1)
```
...
plot_22 <- df_all_hp_best40 |> 
  ...
  scale_color_gradient2(low = bicolor_palette[1],
                        high = bicolor_palette[2],
                        mid = bicolor_palette[1],
                        midpoint = -8,
                        trans = "log",
                        breaks = c(1, 5*1e-3, 1e-5)) +
  facet_grid(name ~ ., scales = "free_y") +
