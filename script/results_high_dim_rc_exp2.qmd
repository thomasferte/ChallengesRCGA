...
We explored to variant of the GA optimizing both the number of features and the MAE :

1) We changed the fitness function from $MAE$ to $MAE + \theta * nbfeatures$. In this case, $\theta$ is a penalty term penalizing genetic individuals selecting many features. We explore $\theta \in \{1e^{-3}, 5*1e^{-3}, 1e^{-2})$.
2) We changed the tournament selection. When selecting new parents, we perform a three individuals tournament (instead of two), then we exclude the individual with the larger number of features and then we select the individual with the lowest $MAE$
3) Finally, we explored an epidemiology guided feature selection where we keep the original GA but we only provide 11 features guided by expert knowledge.
...
df_all <- df_all_temp %>% slice_min(hp_date) %>% mutate(update = "No") %>%
  ...
  ungroup() |> 
  mutate(Penalization = case_when(model == "GeneticSingleIs_GA_epidemio" ~ -1,
         ...
         model = case_when(Penalization == -1 ~ "Epidemio",
...
df_all_hp |> 
  ...
  scale_fill_manual(values = c("#001219", "#005F73", "#0A9396", "#94D2BD", "#EE9B00")) +
...
plot_best_numeric_hp <- df_all_hp_best40 |> 
  ...
  facet_grid(model ~ ., scales = "free_y", switch = "both") +
...
plot_nb_features <- temp |> 
  ...
  scale_color_manual(values = c("#001219", "#005F73", "#0A9396", "#94D2BD", "#EE9B00")) +
